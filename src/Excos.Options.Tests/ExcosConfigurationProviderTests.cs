...
public class ExcosConfigurationProviderTests
{
    ...
    [Fact]
    public void AddExcosConfiguration_WithDictionaryContext_Works()
    {
        // Arrange
        var feature = CreateFeature("TestFeature",
            new Variant
            {
                Id = "V1",
                Configuration = JsonDocument.Parse("""{"App":{"Setting":"FromExcos"}}""").RootElement,
                Filters = []
            });
        ...
        // Assert
        Assert.Equal("FromExcos", config["App:Setting"]);
    }

    [Fact]
    public void AddExcosConfiguration_WithOptionsContext_Works()
    {
        // Arrange
        var feature = CreateFeature("TestFeature",
            new Variant
    ...
    [Fact]
    public void AddExcosConfiguration_CombinesWithOtherSources()
    {
        // Arrange
        var feature = CreateFeature("TestFeature",
            new Variant
            {
                Id = "V1",
                Configuration = JsonDocument.Parse("""{"Excos":{"Value":"FromExcos"}}""").RootElement,
                Filters = []
            });

        var featureProvider = new TestFeatureProvider(feature);

        // Act - Excos added after in-memory, so its values should win on conflict
        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["Memory:Value"] = "FromMemory",
                ["Excos:Value"] = "OverriddenByExcos"
            })
            .AddExcosConfiguration(
                featureProvider,
                new DictionaryOptionsContext(new Dictionary<string, string>()))
        ...
        // Assert
        Assert.Equal("FromMemory", config["Memory:Value"]);
        Assert.Equal("FromExcos", config["Excos:Value"]); // Excos wins
    }
