...
namespace Excos.Options.GrowthBook
{
    internal class GrowthBookFeatureCache : BackgroundService
    {
        ...
        public GrowthBookFeatureCache(IOptionsMonitor<GrowthBookOptions> options, GrowthBookApiCaller growthBookApiCaller, ILogger<GrowthBookFeatureCache> logger)
        {
            _options = options;
        ...
        private async Task RequestFeaturesAsync()
        {
            ...
            try
            {
                ...
                // Note: Default values are now included as variants via ConvertFeaturesToExcos,
                // so we no longer need to update a separate configuration provider
