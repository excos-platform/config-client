...
internal class RangeFilterParser : IFeatureFilterParser
{
    public bool TryParseFilter(IConfiguration configuration, [NotNullWhen(true)] out IFilteringCondition? filteringCondition)
    {
        ...
        {
            filteringCondition = new RangeFilteringCondition<Guid>(guidRange);
            return true;
        }
        if (Range<DateTimeOffset>.TryParse(pattern, null, out var dateRange))
        {
            filteringCondition = new RangeFilteringCondition<DateTimeOffset>(dateRange);
            return true;
        }
        if (Range<double>.TryParse(pattern, null, out var doubleRange))
        {
            filteringCondition = new RangeFilteringCondition<double>(doubleRange);
            return true;
        }
